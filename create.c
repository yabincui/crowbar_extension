                        ...
                        Statement *else_statement)
{
    ...
    st->u.if_s.then_statement = then_statement;
    st->u.if_s.elsif_list = elsif_list;
    st->u.if_s.else_statement = else_statement;
...
Elsif *
crb_create_elsif(Expression *expr, Statement *statement)
{
    Elsif *ei;

    ei = crb_malloc(sizeof(Elsif));
    ei->condition = expr;
    ei->statement = statement;
    ei->next = NULL;
...
Statement *
crb_create_while_statement(Expression *condition, Statement *statement)
{
    Statement *st;

    st = alloc_statement(WHILE_STATEMENT);
    st->u.while_s.condition = condition;
    st->u.while_s.statement = statement;
...
crb_create_for_statement(Expression *init, Expression *cond,
                         Expression *post, Statement *statement)
{
    Statement *st;
    ...
    st->u.for_s.statement = statement;
...
Statement *crb_create_block_statement(Block *block)
{
	...
	st = alloc_statement(BLOCK_STATEMENT);
